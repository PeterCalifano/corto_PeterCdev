...
# TODO 1 implement bpy manager with batch processing mode ("everything-in-python" mode with multi-threading and queues)
# TODO 2 implement automatic scene update to make it static (target fixed) if not so.
# TensorCommManager interfaces may be exploited

raise NotImplementedError("This script is not yet ready for use. Please check the code and remove this line when ready.")
